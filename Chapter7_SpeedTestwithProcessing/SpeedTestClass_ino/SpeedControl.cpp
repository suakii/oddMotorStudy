...
void SpeedControl::adjustPWM()
{  
    enc_speed = (double)_encoder->updateSpeed();
    if (p_pid->ComputeISR())
    {
      if (goal_speed == 0)
      {
      ...
      {
		int16_t pwm_out;
        pwm_out = pwm_output * _dir;      

		if (pwm_out >=0)
        {
		    _motor->setFwd(abs(pwm_out));
        }
	    else
	    {
		    _motor->setBack(abs(pwm_out));
        }        
      }
    }   
}
